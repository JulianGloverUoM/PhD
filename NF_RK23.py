...
def form_equilibrium_matrix(nodes, adjacency_matrix):
    l_j = adjacency_matrix.dot(nodes)
    l_j_hat = normalise_elements(l_j)
    Equilibrium_matrix = np.empty((2 * len(nodes), len(l_j)))
    for k in range(len(nodes)):
        for j in range(len(l_j)):
            Equilibrium_matrix[int(2 * k - 1), j] = adjacency_matrix[j, k] * l_j_hat[j][0]
            Equilibrium_matrix[int(2 * k), j] = adjacency_matrix[j, k] * l_j_hat[j][1]
...
def remove_short_edges(mean_factor, adjacency_matrix, nodes, L):
    edge_lengths = vector_of_magnitudes(adjacency_matrix.dot(nodes))
    iterated_matrix = adjacency_matrix
...
def RK23_timestepper(
    ...
    def scipy_fun(t, y):
        ...
        F_j = np.multiply(1 / initial_lengths, u_j)
        product = np.array([np.multiply(l_j_hat[:, 0], F_j), np.multiply(l_j_hat[:, 1], F_j)])
        f_jk = np.matmul(np.transpose(adjacency_matrix), np.transpose(product))
        f_jk[boundary_nodes:] = 0
        return -np.reshape(f_jk, 2 * np.shape(f_jk)[0], order="C")
...
def remove_short_realisation(
    ...
    (trim_matrix, trim_nodes, trim_boundary_nodes) = remove_short_edges(
        mean_factor, adjacency_matrix, nodes, L
    )
    PBC_network.PlotNetwork_2(trim_nodes, trim_matrix, L, 0)

    print("Size of original system =", np.shape(adjacency_matrix))
...
def discrete_stress(nodes, adjacency_matrix, initial_lengths, L):
    A = L**2
    edge_vectors = adjacency_matrix.dot(nodes)
...
def stress_net_force(nodes, adjacency_matrix, initial_lengths, L):
    l_j = adjacency_matrix.dot(nodes)
    l_j_hat = normalise_elements(l_j)
    l_j_lengths = vector_of_magnitudes(l_j)

    u_j = l_j_lengths - initial_lengths

    product = np.array([np.multiply(l_j_hat[:, 0], u_j), np.multiply(l_j_hat[:, 1], u_j)])
    f_jk = -np.matmul(np.transpose(adjacency_matrix), np.transpose(product))
    stress = np.zeros((2, 2))

    for i in range(len(nodes)):
        stress = stress + np.outer(f_jk[i], nodes[i])

    return -stress / (L**2)


def stress_outer(nodes, adjacency_matrix, initial_lengths, L, boundary_nodes):
    l_j = adjacency_matrix.dot(nodes)
    l_j_hat = normalise_elements(l_j)
    l_j_lengths = vector_of_magnitudes(l_j)
    ...
    F_j = u_j / initial_lengths
    boundary_edges = []
    for i in range(len(nodes) - boundary_nodes):
        boundary_edges.append(np.nonzero(adjacency_matrix[:, i + boundary_nodes])[0][0])

    stress = np.zeros((2, 2))
    for i in range(len(boundary_edges)):
        k = boundary_nodes + i
        j = boundary_edges[i]
        stress = stress + np.outer(adjacency_matrix[j, k] * F_j[j] * l_j_hat[j], nodes[k])
