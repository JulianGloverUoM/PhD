...
def uniaxial_stretch(input_nodes, stretch_factor):
    num_nodes = np.shape(input_nodes)[0]
    output = np.empty([num_nodes, 2], dtype=float)
    deformation = np.array([[1, 0], [0, 1 + stretch_factor]])
    for i in range(num_nodes):
        output[i] = deformation.dot(input_nodes[i])
    return output


def invert_uniaxial_stretch(input_nodes, stretch_factor):
    num_nodes = np.shape(input_nodes)[0]
    output = np.empty([num_nodes, 2], dtype=float)
    deformation = np.array([[1, 0], [0, 1 / (1 + stretch_factor)]])
    for i in range(num_nodes):
        output[i] = deformation.dot(input_nodes[i])
    return output
...
def Radau_timestepper_uniaxial_stretch(
    ...
    y = uniaxial_stretch(nodes, stretch_factor)
    ...
    if Plot_networks:
        try:
            PBC_network.ColormapPlot_uniaxial_stretch(
...
def Single_realisation_uniaxial_stretch(
    L,
    density,
    seed,
    ...
    for i in range(num_intervals):
        if i == 0 and fibre_lengths_multiplier == 1:
            ...
            continue
        data.append(
            Radau_timestepper_uniaxial_stretch(
                L,
                input_nodes,
                incidence_matrix,
                boundary_nodes,
        ...
        input_nodes = invert_uniaxial_stretch(
    ...
    if Plot_stress_results:
        strains = np.linspace(1, 1 + stretch_stepsize * (num_intervals - 1), num_intervals)
        if fibre_lengths_multiplier == 1:
            strains = np.linspace(
                1 + stretch_stepsize, 1 + stretch_stepsize * (num_intervals - 1), num_intervals - 1
            )
        plt.figure()
...
def Edge_lengths_and_orientations_histogram_plot(
    ...
    plt.title(r"PDF of edge length distribution $\lambda$ = {}".format(strain))

    plt.figure()
    plt.hist(orientations, bins=bins_orientations, density=True)
    plt.title(r"PDF of fibre orientations for $\lambda$ = {}".format(strain))
    return
...
def Stretch_distribution(nodes, incidence_matrix, initial_lengths, num_bins, strain):
    ...
    plt.title(r"PDF of fibre stretches for $\lambda$ = {}".format(strain))
    return
...
def affinity_strain_stretch(initial_nodes, input_nodes, incidence_matrix, stretch_factor):
    ...
    affine_nodes = uniaxial_stretch(initial_nodes, stretch_factor)
