...
# -*- coding: utf-8 -*-
"""
Created on Tue Sep 16 10:37:22 2025
...
def Radau_timestepper_dilation(
    ...
    def scipy_fun(t, y):
        ...
        l_j_hat = normalise_elements(l_j)
        F_j = (1 / 6) * (
            (np.sqrt(np.einsum("ij,ij->i", l_j, l_j)) / initial_lengths) ** 6 - initial_lengths
        )
        product = np.einsum("ij,i->ij", l_j_hat, F_j)
        f_jk = incidence_matrix.T.dot(product)
        f_jk[boundary_nodes:] = 0
    ...
    def energy_calc(y):
        matrix_y = np.reshape(y, (np.shape(incidence_matrix)[1], 2))
        l_j = vector_of_magnitudes(incidence_matrix.dot(matrix_y))
        lambda_j = l_j / initial_lengths
        return sum((1 / 42) * (lambda_j**7 - 7 * lambda_j + 6))
    ...
    if Plot_networks:
        try:
            Fixed_BC_script.ColormapPlot_dilation(
                ...
                (1 / 6)
                * (
                    (vector_of_magnitudes(incidence_matrix.dot(y_output)) / initial_lengths) ** 6
                    - 1
                ),
                r"$F_j$",
            )
