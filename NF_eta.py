...
def remove_short_edges(mean_factor, adjacency_matrix, nodes, L):
    ...
    for i in range(len(iterated_nodes)):
        if any(iterated_nodes[i] % L == 0):
            boundary_nodes = i
    return (iterated_matrix, iterated_nodes, boundary_nodes)
...
def RK23_eta_timestepper(
    ...
    def scipy_fun(t, y):
        ...
        f_jk[boundary_nodes:] = 0
        f_jk = np.transpose(
            np.array([np.multiply(f_jk[:, 0], eta_scale), np.multiply(f_jk[:, 1], eta_scale)])
        )
        return -np.reshape(f_jk, 2 * np.shape(f_jk)[0], order="C")
    ...
    eta_scale_list = []
    for i in range(np.shape(adjacency_matrix)[1]):
        incident_edges = np.nonzero(adjacency_matrix[:, i])[0]
        eta_scale_list.append(min(initial_lengths_inv[index] for index in incident_edges))

    eta_scale = np.array(eta_scale_list)
    y_values = []
    t_values = []
...
def Single_realisation_eta(
    ...
    for i in range(num_intervals):
        if fibre_lengths_multiplier == 1 and i == 0:
            continue
        data.append(
            RK23_eta_timestepper(
                L,
                input_nodes,
                adjacency_matrix,
                boundary_nodes,
...
def Many_realisations_eta(
    ...
    for i in range(num_realisations):
        ...
        (realisation_data, realisation_stress_strains) = Single_realisation_eta(
            L,
            density,
            i,
            fibre_lengths_multiplier,
